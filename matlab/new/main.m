...
success_bin_nums = num2cell(floor((successes - emin) ./ bin_size) + 1);
failure_bin_nums = num2cell(floor((failures - emin) ./ bin_size) + 1);
...
for k = 1:NUM_POINTS
    success_bins(success_bin_nums{:,k}) = success_bins(success_bin_nums{:,k}) + 1;
    failure_bins(failure_bin_nums{:,k}) = failure_bins(failure_bin_nums{:,k}) + 1;
end
